...
# target 列表
target_df_train = pd.read_csv('target.csv')
target_df_val = pd.read_csv('target_val.csv')
...
for root, dirs, files in os.walk(train_data_dir):
    for filename in files:
        ...
        if filename[-4:] != '.svs':
            continue
        for file in target_df_train['slide']:
            if file==filename:
                flag = True
                continue
        if not flag :
        ...
        print(filename)
        train_targets_list.append((int)(target_df_train[target_df_train['slide'] == filename]['target'].values[0]))
...
for root, dirs, files in os.walk(val_data_dir):
    for filename in files:
        ...
        if filename[-4:] != '.svs':
            continue
        for file in target_df_val['slide']:
            if file==filename:
                flag = True
                continue
        if not flag :
        ...
        print(filename)
        val_targets_list.append((int)(target_df_val[target_df_val['slide'] == filename]['target'].values[0]))
