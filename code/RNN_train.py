...
def train_single(epoch, embedder, rnn, loader, criterion, optimizer):
    ...
    train_err = (running_fps + running_fns)/2
    print('Training - Epoch: [{}/{}]\tLoss: {}\tFPR: {}\tFNR: {}\tACC:{}'.format(epoch+1, args.nepochs, running_loss, running_fps, running_fns,1-train_err))
...
def test_single(epoch, embedder, rnn, loader, criterion):
    ...
    running_fps = running_fps/(np.array(loader.dataset.targets)==0).sum()
    running_fns = running_fns/(np.array(loader.dataset.targets)==1).sum()
    test_err = (running_fps + running_fns)/2
    print('Validating - Epoch: [{}/{}]\tLoss: {}\tFPR: {}\tFNR: {}\tACC:{}'.format(epoch+1, args.nepochs, running_loss, running_fps, running_fns,1-test_err))
    return running_loss, running_fps, running_fns
